...
def transform(args, workspace_lh ,workspace_rh, align_data_lh, align_data_rh, nsubjs):
  ...
  if args.align_algo in ['srm_noneprob_vary_voxels', 'ha_ana_orth_vary']:
    ...
    for m in range(nsubjs):
      nvoxel = align_data_lh[m].shape[0]
      transform_lh[m] = zeros((nvoxel, args.nfeature))
      transform_rh[m] = zeros((nvoxel, args.nfeature))
  else:
  ...
  if args.align_algo in ['ha', 'srm_noneprob']:
  ...
  elif args.align_algo in ['srm_noneprob_vary_voxels', 'ha_ana_orth_vary']:
    ...
    for m in range(nsubjs-1):
      ...
      transform_rh[loo_idx[m]] = bW_rh[m]
    # find transform_lh[:,:,loo], transform_rh[:,:,loo]
    Am = align_data_lh_loo_zscore.dot(workspace_lh['S'])
    pert = np.zeros((Am.shape)) 
    np.fill_diagonal(pert,1)
    U_lh, s_lh, V_lh = np.linalg.svd(align_data_lh_loo_zscore.dot(workspace_lh['S'])+0.001*pert, full_matrices=False)
    U_rh, s_rh, V_rh = np.linalg.svd(align_data_rh_loo_zscore.dot(workspace_rh['S'])+0.001*pert, full_matrices=False)
    transform_lh[loo] = U_lh.dot(V_lh)
    transform_rh[loo] = U_rh.dot(V_rh)
